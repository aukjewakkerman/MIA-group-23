...
def combining_transforms():
    ...
    X = util.test_object(1)
    X_combi = reg.reflect(-1,1).dot(reg.rotate(np.pi/2)).dot(X)
    fig = plt.figure(figsize=(12,5))
    ax1 = fig.add_subplot(141, xlim=(-4,4), ylim=(-4,4))
    util.plot_object(ax1, X_combi)
    ax1.set_title('combi')
    plt.show()
    #------------------------------------------------------------------#
...
def arbitrary_rotation():
    ...
    t = X[:,0]
    T1 = util.t2h(np.eye(2), -t)
    R = np.eye(3)
    R[:2, :2] = reg.rotate(np.pi/4)
    T2 = util.t2h(np.eye(2), t)
    T = T2 @ R @ T1
    X_rot = T.dot(Xh)
    #------------------------------------------------------------------#
...
def ls_affine_test():
    ...
    # moving image
    Xm = T.dot(Xh)
    # least squares fitting fixed and moving image
...
def correlation_test():
    ...
    I = plt.imread('../data/cameraman.tif')
    T_rot = reg.rotate(np.pi/4)
    Th = util.t2h(T_rot, np.array([0,0]))   # for self-corr the identity matrix
    ...
    # another assumption is the linear relationship between pixel intensities in images
    #------------------------------------------------------------------#
...
def mutual_information_test():
    ...
    #I = plt.imread('../data/cameraman.tif')
    I_1 = np.random.randint(255, size=(512, 512))
    I_1_same = I_1.copy()
    I_2 = np.random.randint(255, size=(512, 512))

    #------------------------------------------------------------------#
    # TODO: Implement a few tests of the mutual_information definition
    # Image with itself: should give high MI
    p_same_image = reg.joint_histogram(I_1, I_1_same)
    MI_same_image= reg.mutual_information(p_same_image)
    print(f"MI(image, image) = {MI_same_image:.4f} (should be high)")

    # Image with a random image
    p_rand = reg.joint_histogram(I_1, I_2)
    MI_rand = reg.mutual_information(p_rand)
    print(f"MI(image, random_image) = {MI_rand:.4f} (should be low)")
    #------------------------------------------------------------------#
...
def registration_metrics_demo(use_d=True, use_t2=False):

    # read a T1 image
    I = plt.imread('../data/image_data/1_1_t1.tif')
    if use_d:
        I_t1 = plt.imread('../data/image_data/1_1_t1_d.tif')
    
    if use_t2:
        # read the corresponding T2 image
        # note that the T1 and T2 images are already registered
        I_t2 = plt.imread('../data/image_data/1_1_t2.tif')
    ...
    ax2.grid()
    plt.title("Rigid intensity-based registration of T1 vs T1")
    ...
    for k, ang in enumerate(angles):
        ...
        if use_d:
            J, Xt = reg.image_transform(I_t1, T_rot)
        else:
            # rotate the T1 image
