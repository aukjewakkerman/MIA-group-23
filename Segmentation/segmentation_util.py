...
def scatter_data(X, Y, features, feature0=0, feature1=1, ax=None):
    ...
    if len(X) > k:
        ...
        X = X[idx,:]
        Y = Y[idx]
        # each element in Y corresponds to a data point in X and represents its class label

    class_labels, indices1, indices2 = np.unique(Y, return_index=True, return_inverse=True)
    # indices2 gives the index to class_labels and maps every value of Y to a class index
    if ax is None:
        fig = plt.figure(figsize=(8,8))
        ax = fig.add_subplot(111)
        ax.grid()
        ax.set_title(f'{features[feature0]} vs {features[feature1]}')

    labels = ['white matter', ' gray matter', 'csf']
    ...
    ax.set_xlabel(features[feature0])
    ax.set_ylabel(features[feature1])
    ax.legend()

    return ax


def create_dataset(image_number, slice_number, task, filter=False, methods=['none']):
    ...
    X, feature_labels = extract_features(image_number, slice_number, methods)

    #Create labels
    Y = create_labels(image_number, slice_number, task)
    
    # if applicable, background filter
    if filter:
        mask = Y != 0
        Y_filtered = Y[mask]
        Y_filtered = Y_filtered.copy()  # Avoid modifying a view of original Y
        Y_filtered[Y_filtered == 1] = 0  # white matter → 0
        Y_filtered[Y_filtered == 2] = 1  # gray matter → 1
        Y_filtered[Y_filtered == 3] = 2  # csf → 2
    
        X_filtered = X[mask.flatten(), :]  # assuming X has shape (pixels, features)
        
        return X_filtered, Y_filtered, feature_labels
...
def extract_features(image_number, slice_number, methods=['none']):
    ...
    if 'gaussian' in methods:
        t1 = project.apply_gaussian_smoothing(t1, sigma=sigma)
        t2 = project.apply_gaussian_smoothing(t2, sigma=sigma)
        print('gaussian selected')
    if 'histogram' in methods:
        t1 = project.apply_histogram_equalization(t1, sigma=sigma)
        t2 = project.apply_histogram_equalization(t2, sigma=sigma)
    ...
    #blurred_t1 = ndimage.gaussian_filter(t1, sigma=20)
    #blurred_t1 = blurred_t1.flatten().T
    #blurred_t1 = blurred_t1.reshape(-1, 1)
    #X = np.concatenate((X, blurred_t1), axis=1)
    #features += ('T1 blur ' + str(20),)
...
def classification_error(true_labels, predicted_labels):
    ...
    #------------------------------------------------------------------#
    # TODO: Implement the midissing functionality for classification error
    #!studentstart
