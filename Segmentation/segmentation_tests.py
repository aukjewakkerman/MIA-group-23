...
def scatter_data_test(showFigs=True):
    ...
    if showFigs:
        util.scatter_data(X_data,Y,features,0,1)
    ...
    # Keep track of features you added
    features = ('T1 intensity', 'T1 gauss 2', 'T1 gauss 4', 'T1 gauss 10')
    if showFigs:
        util.scatter_data(X_data,Y,features,0,2)
        util.scatter_data(X_data,Y,features,0,3)
...
def scatter_t2_test(showFigs=True):
    ...
    if showFigs:
        util.scatter_data(X_data,Y,features,2,3)
        util.scatter_data(X_data,Y,features,0,2)
        util.scatter_data(X_data,Y,features,1,3)
...
def distance_test():
    ...
    ax = util.scatter_data(X,Y,features)
...
def kmeans_demo():
    ...
    ax1.grid()
    ax2  = fig.add_subplot(122, xlim=(0, num_iter), ylim=(0,1000))
    ...
    for k in np.arange(num_iter):
        ...
        kmeans_cost[k] = seg.cost_kmeans(X, w_vector)
        print('for k={}, cost={:.2f}'.format(k, kmeans_cost[k]))
...
def kmeans_clustering_test():
    ...
    test_data, test_labels, feature_labels = util.create_dataset(3,1,'brain',filter=False)
