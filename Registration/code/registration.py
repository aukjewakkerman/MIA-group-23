...
def rotate(phi):
    ...
    T = np.array([[np.cos(phi), -np.sin(phi)],
        [np.sin(phi),  np.cos(phi)]]) # dummy matrix, to be replaced.
    #------------------------------------------------------------------#
    return T
...
def shear(cx, cy):
    ...
    T = np.array([[1,cx],
                  [cy,1]]) # dummy matrix, to be replaced.
    #------------------------------------------------------------------#
...
def reflect(rx, ry):
    ...
    T = np.array([[1 - 2*(rx**2)/norm, -2*rx*ry/norm],
        [-2*rx*ry/norm, 1 - 2*(ry**2)/norm]]) # dummy matrix, to be replaced.
    #------------------------------------------------------------------#
...
def ls_affine(X, Xm):
    ...
    # Xm - Corresponding points in the moving image, this is 2xn
    # Output:
    ...
    T = np.eye(3)
    # Add a row of ones to Xm to make it homogeneous
    Xm_h = np.zeros((3, Xm.shape[1]))
    Xm_h[:2, :] = Xm
    Xm_h[2, :] = 1
    ...
    w_x = ls_solve(Xm_h.T, x_points)
    w_y = ls_solve(Xm_h.T, y_points)
    T[0, :] = w_x
    T[1, :] = w_y
    #------------------------------------------------------------------#
...
def joint_histogram(I, J, num_bins=16, minmax_range=None):
    ...
    # (p.m.f.).
    # you have to go from counts to probability
    p = p / np.sum(p)
    #------------------------------------------------------------------#
...
def mutual_information(p):
    ...
    # p - joint histogram
    # Output:
    # MI - mutual information in bit units (because log2)
    # a very small positive number
    # Higher MI better alignment (for bits typically between 0 and 7)
...
def affine_corr(I, Im, x, return_transform=True):
    ...
    R = rotate(x[0])
    A = np.array([
        [x[1], x[3]],   # scale_x, shear_x
        [x[4], x[2]]    # shear_y, scale_y
    ])
    RS = R @ A  # 2×2
    T = np.eye(3)
    T[0:2, 0:2] = RS
    T[0:2, 2] = [x[5], x[6]]  # translation
    Th = T
    Im_t, Xt = image_transform(Im, Th)
...
def affine_mi(I, Im, x, return_transform=True):
    ...
    R = rotate(x[0])
    A = np.array([
        [x[1], x[3]],   # scaling on 0,0 and 1,1
        [x[4], x[2]]    # shearing on 0,1 and 1,0
    ])
    RS = R @ A  # 2×2
    T = np.eye(3)
    T[0:2, 0:2] = RS
    T[0:2, 2] = np.array([x[5], x[6]]) * SCALING # translation
    Th = T
    Im_t, Xt = image_transform(Im, Th)
